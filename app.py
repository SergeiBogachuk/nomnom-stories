...
if check_password():
    ...
    # Инициализация темы в памяти
    if 'theme_idx' not in st.session_state: st.session_state.theme_idx = 0

    # Настройки в боковой панели
    with st.sidebar:
        st.success(f"Привет, **{st.session_state['display_name']}**!")
    ...
    details = st.text_area("✍️ Добавь деталей (о чем сказка?)")
            ...
            try:
                ...
                for i in range(num_chapters):
                    ...
                    ch_res = client.chat.completions.create(
                        model="gpt-4o",
                        messages=[{"role": "user", "content": f"Напиши главу {i+1} сказки для {child_name} на тему {curr_theme}. Ситуация: {details}. Язык: {lang}."}]
                    )
